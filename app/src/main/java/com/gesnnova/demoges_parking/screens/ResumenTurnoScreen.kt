...
import com.gesnnova.demoges_parking.viewmodels.ResumenTurnoViewModel
...
fun ResumenTurnoScreen(navController: NavController, sessionViewModel: SessionViewModel){
    ...
    LaunchedEffect(cierreParaImprimir) {
        cierreParaImprimir?.let { cierre ->
            resumen?.let {
                printViewModel.imprimirCierre(context = context,
                    data = cierre,
                    efectivoAbono = it.abonoEfectivo,
                    tarjetaAbono = it.abonoTarjeta,
                    transferenciaAbono = it.abonoTransferencia
                )
            }
                ...
                resumen?.let {
                    Card(
                        ...
                        Column(
                            ...
                            InfoRow("Efectivo abono", "$${it.abonoEfectivo}")
                            InfoRow("Tarjeta abono", "$${it.abonoTarjeta}")
                            InfoRow("Transferencia abono", "$${it.abonoTransferencia}")
                            InfoRow("Total abonos", "$${it.totalAbonos}")
                            Divider(modifier = Modifier.padding(vertical = 8.dp))
